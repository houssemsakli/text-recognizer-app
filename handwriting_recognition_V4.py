...
def heavy_processing(gray):
    ...
    print(f'Heavy processing PID: {os.getpid()}, PPID: {os.getppid()}')
...
def fast_processing(gray):
    ...
    def worker(queue, img_part):
        print(f'Thread PID: {os.getpid()}, PPID: {os.getppid()}')
    ...
    parts = [gray[i * height // num_threads: (i + 1) * height // num_threads, :] for i in range(num_threads)]
    ...
    print(f'Fast processing used {num_threads} threads.')
